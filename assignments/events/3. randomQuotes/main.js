...
var rq = document.querySelector(".randquote");
var ra = document.querySelector(".randauthor");
function quoteSelector(elem) {
  if (elem.keyCode == 32) {
    event = Math.floor(Math.random() * quotes.length);
    rq.innerText = `"${quotes[event].quoteText}"`;
    ra.innerText = `${quotes[event].quoteAuthor}`;
  }
}
document.addEventListener("keydown", quoteSelector);
